...
void change_motor_state()
{
	...
	if(motor_off || motor_emergency_stop_flag || (!(back_emf_PORTC_state & 0x08) && !(back_emf_PORTC_state & 0x10) && !(back_emf_PORTC_state & 0x20)) ) { 
	...
	if(motor_forwards) {
		...
		if(expected_motor_state == 1 && back_emf_PORTC_state & _BV(4)) {
		...
		if(expected_motor_state == 2 && back_emf_PORTC_state & _BV(5)) {
		...
		if(expected_motor_state == 3 && back_emf_PORTC_state & _BV(3)) {
		...
		if(expected_motor_state == 1 && back_emf_PORTC_state & _BV(3)) {
		...
		if(expected_motor_state == 3 && back_emf_PORTC_state & _BV(4)) {
